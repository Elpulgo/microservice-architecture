    ...
    pub fn add_batch(&mut self, batch: Batch) {
        ...
        match self.batch_handler_map.entry(String::from(key)) {
            ...
            Entry::Occupied(mut e) => {
                match e.get_mut().is_timeout_exceeded() {
                    true => timeout_exceeded = true,
                    false => {
                        match batch.is_last_in_batch() {
                            true => {
                                let batch_size = batch.batch_size;
                                e.get_mut().add_batch_value(batch);
                                match batch_size == e.get().batches.len() {
                                    true => {
                                        println!("Should update status to PendingDatabase and do redis insert..");
                                        e.get_mut().change_status(BatchStatus::PendingDatabase);
                                        let ref_batches = &e.into_mut().batches;
                                        match redis_manager::set_hash_all(String::from(key), ref_batches) {
                                            Ok(res) => {}
                                            Err(err) => {}
                                        }
                                    ...
                                    false => {
                                        ...
                                        e.get_mut().change_status(BatchStatus::Invalid);
...
impl BatchHandler {
    pub fn is_timeout_exceeded(&mut self) -> bool {
